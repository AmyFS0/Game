...
const menu=$('menu'),hud=$('hud'),endScreen=$('endScreen'),panel=$('panel'),toast=$('toast'),crosshair=$('crosshair');
...
addEventListener('keydown',e=>{keys[e.key.toLowerCase()]=true;if(e.code==='Space')e.preventDefault();ability(e)});
...
{name:'THE RIFT',accent:'#f472b6',bg:'#0b0310',glow:'rgba(244,114,182,.24)',waves:3}
];

let state='menu',last=performance.now(),score=0,kills=0,level=1,wave=1,shake=0,flash=0;
let bullets=[],enemies=[],particles=[],pickups=[],rings=[],stars=[];
let spawnRemaining=0,spawnCooldown=0,clearTimer=-1,bossSpawned=false,pickupTimer=7;
...
function notify(msg){toast.textContent=msg;toast.classList.add('show');clearTimeout(notify.t);notify.t=setTimeout(()=>toast.classList.remove('show'),1500)}
...
function startGame(){state='play';menu.classList.add('hidden');endScreen.classList.add('hidden');hud.classList.remove('hidden');panel.classList.add('hidden');score=0;kills=0;level=1;wave=1;bullets=[];enemies=[];particles=[];pickups=[];rings=[];pickupTimer=5;Object.assign(player,{x:W*.35,y:H*.58,hp:100,energy:100,weapon:0,lastShot:0,dashCd:0,shieldCd:0,overCd:0,shield:0,over:0,bulletShield:0,inv:0});beginLevel(1);notify('RIFT GATE OPEN // LEVEL 1')}
$('playBtn').onclick=startGame;$('restartBtn').onclick=startGame;

function beginLevel(n){level=n;wave=1;player.energy=100;if(n>1)player.hp=Math.min(100,player.hp+15);startWave();updateHUD()}
function startWave(){spawnRemaining=4+level*2+wave*2+(level===5?2:0);spawnCooldown=.25;clearTimer=-1;bossSpawned=false;notify(`${levels[level-1].name} // WAVE ${wave}`)}
function advance(){const cfg=levels[level-1];if(wave<cfg.waves){wave++;startWave()}else if(level<5){beginLevel(level+1);notify(`LEVEL ${level} // ${levels[level-1].name}`)}else finish(true)}
...
if(kind===3){const hp=1650;enemies.push({type:'RIFT TITAN',x:W+80,y:H*.45,r:52,hp,maxHp:hp,speed:45,damage:24,color:'#f472b6',shoot:true,boss:true,lastAttack:0,phase:0});notify('⚠ FINAL BOSS // RIFT TITAN');return}
const presets=[
...
const maxType=Math.min(2,Math.floor(level/2));
const p=presets[kind===null?Math.floor(Math.random()*(maxType+1)):kind];
const finalBoost=level===5?1.18:1,hpScale=(1+(level-1)*.24)*finalBoost,speedScale=1+(level-1)*.075,damageScale=1+(level-1)*.12;
...
function enemyShoot(e,now){const baseRate=e.boss?540:e.type==='SNIPER'?1050:1350,rate=baseRate*Math.max(.58,1-(level-1)*.07);if(now-e.lastAttack<rate)return;e.lastAttack=now;const a=Math.atan2(player.y-e.y,player.x-e.x),n=e.boss?7:(level===5&&e.type==='SNIPER'?3:1);for(let i=0;i<n;i++){const off=(i-(n-1)/2)*(e.boss?.13:.11);const speed=e.boss?390:305+(level-1)*8;bullets.push({x:e.x,y:e.y,vx:Math.cos(a+off)*speed,vy:Math.sin(a+off)*speed,r:e.boss?6:4,life:3.5,damage:e.damage,color:e.color,enemy:true})}}
...
function collect(p){if(p.type==='shield'){player.bulletShield=8;ring(p.x,p.y,'#facc15');burst(p.x,p.y,'#facc15',22,5);notify('YELLOW ORB // BULLET PROTECTION 8s')}else{const old=player.hp;player.hp=Math.min(100,player.hp+30);ring(p.x,p.y,'#38bdf8');burst(p.x,p.y,'#38bdf8',22,5);notify(old>=100?'BLUE ORB // HEALTH FULL':'BLUE ORB // +30 HP')}p.dead=true;score+=150}

function ability(e){if(state!=='play')return;if(['1','2','3'].includes(e.key)){player.weapon=+e.key-1;updateHUD();notify(weapons[player.weapon].name)}if(e.code==='Space'&&player.dashCd<=0&&player.energy>=18){let dx=(keys.d?1:0)-(keys.a?1:0),dy=(keys.s?1:0)-(keys.w?1:0);if(!dx&&!dy){const a=Math.atan2(mouse.y-player.y,mouse.x-player.x);dx=Math.cos(a);dy=Math.sin(a)}const l=Math.hypot(dx,dy)||1;player.x+=dx/l*130;player.y+=dy/l*130;player.energy-=18;player.dashCd=1.3;player.inv=.25;ring(player.x,player.y,'#22d3ee');burst(player.x,player.y,'#22d3ee',18,5)}if(e.key.toLowerCase()==='q'&&player.shieldCd<=0&&player.energy>=30){player.energy-=30;player.shield=3.5;player.shieldCd=8;ring(player.x,player.y,'#7c3aed');notify('ENERGY SHIELD ONLINE')}if(e.key.toLowerCase()==='e'&&player.overCd<=0&&player.energy>=35){player.energy-=35;player.over=4;player.overCd=10;notify('OVERCHARGE // 150%')}}

function damagePlayer(d,bullet=false){if(player.shield>0){burst(player.x,player.y,'#7c3aed',8,3);return}if(bullet&&player.bulletShield>0){burst(player.x,player.y,'#facc15',9,3);ring(player.x,player.y,'#facc15');return}player.hp-=d;shake=8;flash=.15;burst(player.x,player.y,'#fb7185',12,4);if(player.hp<=0)finish(false)}
function killEnemy(e,index){enemies.splice(index,1);kills++;score+=e.boss?3500:100+level*20;shake=e.boss?12:4;ring(e.x,e.y,e.color);burst(e.x,e.y,e.color,e.boss?55:16,e.boss?8:4);if(!e.boss&&Math.random()<.18)spawnPickup(Math.random()<.55?'shield':'health',e.x,e.y);if(e.boss)notify('RIFT TITAN DEFEATED')}

function update(dt,now){if(state!=='play')return;
let dx=(keys.d?1:0)-(keys.a?1:0),dy=(keys.s?1:0)-(keys.w?1:0),len=Math.hypot(dx,dy)||1,sp=player.speed*(keys.shift?1.28:1)*(player.over?1.15:1);
...
player.energy=Math.min(100,player.energy+8*dt);player.dashCd-=dt;player.shieldCd-=dt;player.overCd-=dt;player.shield-=dt;player.over-=dt;player.bulletShield-=dt;player.inv-=dt;

if(spawnRemaining>0){spawnCooldown-=dt;if(spawnCooldown<=0){spawnEnemy();spawnRemaining--;spawnCooldown=Math.max(.25,.78-level*.075)}}
const finalBossWave=level===5&&wave===levels[4].waves;
if(finalBossWave&&spawnRemaining===0&&!bossSpawned&&enemies.length===0){bossSpawned=true;spawnEnemy(3)}
...
const cleared=spawnRemaining===0&&enemies.length===0&&(!finalBossWave||bossSpawned);if(cleared&&clearTimer<0)clearTimer=1.25;if(clearTimer>=0){clearTimer-=dt;if(clearTimer<=0)advance()}updateHUD()}

function finish(win){state='end';hud.classList.add('hidden');endScreen.classList.remove('hidden');$('endEyebrow').textContent=win?'CAMPAIGN COMPLETE':'RIFT COLLAPSE';$('endTitle').innerHTML=win?'RIFT<br>CONQUERED':'SIGNAL<br>LOST';$('endText').textContent=win?'The Rift Titan is defeated. All five dimensions are stable.':'Nova was forced out of the fracture. Re-enter the Rift.';$('finalScore').textContent=score.toLocaleString();$('finalLevel').textContent=Math.min(level,5)+'/5';$('finalKills').textContent=kills}

function updateHUD(){const cfg=levels[level-1];$('healthBar').style.width=Math.max(0,player.hp)+'%';$('healthText').textContent=Math.max(0,Math.ceil(player.hp));$('energyBar').style.width=player.energy+'%';$('energyText').textContent=Math.ceil(player.energy);$('score').textContent=String(score).padStart(6,'0');$('killsHud').textContent=kills;$('levelBadge').textContent='LV.'+String(level).padStart(2,'0');$('waveText').textContent=`LEVEL ${level} · WAVE ${wave}/${cfg.waves}`;$('objective').textContent=level===5&&wave===cfg.waves?'DEFEAT THE RIFT TITAN':'CLEAR THE RIFT WAVE';$('dimensionLabel').textContent='DIMENSION // '+cfg.name;const w=weapons[player.weapon];$('weaponName').textContent=w.name;$('weaponRarity').textContent=w.rarity;$('weaponSlot').textContent='0'+(player.weapon+1);$('dashStatus').textContent=player.dashCd>0?player.dashCd.toFixed(1)+'s':'READY';$('shieldStatus').textContent=player.shield>0?'ACTIVE':player.shieldCd>0?player.shieldCd.toFixed(1)+'s':'READY';$('overchargeStatus').textContent=player.over>0?'ACTIVE':player.overCd>0?player.overCd.toFixed(1)+'s':'READY';const o=$('orbStatus');o.classList.toggle('active',player.bulletShield>0);o.textContent=player.bulletShield>0?'● BULLET SHIELD '+Math.max(0,player.bulletShield).toFixed(1)+'s':'NO BULLET SHIELD'}

function drawBackground(t){const cfg=levels[level-1]||levels[0];ctx.fillStyle=cfg.bg;ctx.fillRect(0,0,W,H);const g=ctx.createRadialGradient(W*.72,H*.48,20,W*.72,H*.48,W*.62);g.addColorStop(0,cfg.glow);g.addColorStop(.46,'rgba(34,211,238,.035)');g.addColorStop(1,'rgba(5,5,16,0)');ctx.fillStyle=g;ctx.fillRect(0,0,W,H);for(let i=0;i<stars.length;i++){const s=stars[i];s.x-=(.1+s.z*.26);if(s.x<0)s.x=W;ctx.globalAlpha=.18+s.z*.62;ctx.fillStyle=i%6===0?cfg.accent:'#dff8ff';ctx.fillRect(s.x,s.y,s.s,s.s)}ctx.globalAlpha=1;const horizon=H*.72;ctx.strokeStyle=cfg.accent+'18';for(let y=horizon;y<H;y+=28){ctx.beginPath();ctx.moveTo(0,y);ctx.lineTo(W,y);ctx.stroke()}for(let x=-W;x<W*2;x+=70){ctx.beginPath();ctx.moveTo(W/2+(x-W/2)*.12,horizon);ctx.lineTo(x,H);ctx.stroke()}if(level===1){for(let i=0;i<8;i++){const x=(i*173+80)%W,h=70+(i%4)*38;ctx.fillStyle='rgba(8,13,30,.85)';ctx.fillRect(x,horizon-h,78,h)}}else if(level===2){for(let i=0;i<10;i++){const x=(i*137+60)%W,y=140+(i%4)*95;ctx.fillStyle=cfg.accent+'22';ctx.strokeStyle=cfg.accent+'66';ctx.beginPath();ctx.moveTo(x,y-34);ctx.lineTo(x+18,y);ctx.lineTo(x,y+38);ctx.lineTo(x-18,y);ctx.closePath();ctx.fill();ctx.stroke()}}else if(level===3){for(let i=0;i<9;i++){const x=(i*181+40)%W;ctx.strokeStyle='rgba(52,211,153,.18)';ctx.lineWidth=5;ctx.beginPath();ctx.moveTo(x,H);ctx.quadraticCurveTo(x+60,H*.55,x+20,90);ctx.stroke()}}else if(level===4){for(let i=0;i<6;i++){const x=(i*220+90)%W,y=180+(i%2)*160;ctx.fillStyle='rgba(96,165,250,.1)';ctx.beginPath();ctx.ellipse(x,y,80,20,0,0,Math.PI*2);ctx.fill()}}else{ctx.save();ctx.translate(W*.75,H*.38);ctx.rotate(t*.00012);ctx.strokeStyle=cfg.accent+'66';for(let r=70;r<155;r+=19){ctx.beginPath();ctx.arc(0,0,r,0,Math.PI*1.6);ctx.stroke()}ctx.restore()}}
...
const panelData={characters:{title:'CHARACTER',html:'<div class="grid-cards"><div class="mini-card"><small>RIFT RUNNER</small><b>NOVA</b><p>Fast dimensional explorer with dash, shield and overcharge abilities.</p></div></div>'},loadout:{title:'LOADOUT',html:weapons.map((w,i)=>`<div class="mini-card"><small>${w.rarity}</small><b>0${i+1} · ${w.name}</b><p>Damage ${w.damage}</p></div>`).join('')},missions:{title:'LEVELS',html:levels.map((l,i)=>`<div class="mini-card"><small>LEVEL 0${i+1}</small><b>${l.name}</b><p>${i===4?'FINAL LEVEL · Rift Titan · maximum difficulty':'Clear every wave to unlock the next dimension.'}</p></div>`).join('')},help:{title:'SURVIVAL ORBS',html:'<div class="grid-cards"><div class="mini-card"><small>YELLOW ORB</small><b>BULLET PROTECTION</b><p>Blocks enemy projectiles for 8 seconds.</p></div><div class="mini-card"><small>BLUE ORB</small><b>HEALTH +30</b><p>Restores up to 30 HP.</p></div><div class="mini-card"><small>CONTROLS</small><b>WASD + MOUSE</b><p>Click fire · 1/2/3 weapons · Space dash · Q shield · E overcharge.</p></div></div>'}};
document.querySelectorAll('[data-panel]').forEach(b=>b.onclick=()=>{const d=panelData[b.dataset.panel];$('panelTitle').textContent=d.title;$('panelContent').innerHTML=d.html;panel.classList.remove('hidden')});$('closePanel').onclick=()=>panel.classList.add('hidden');
